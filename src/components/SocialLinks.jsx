...
import { Instagram, Twitter, Youtube, Facebook, Linkedin, Github, Mail, Phone, Globe, Send, MessageCircle } from 'lucide-react';
...
const SocialLinks = ({ links }) => {
  if (!links || links.length === 0) {
    return <p className="text-center text-muted-foreground">Nenhum link social configurado.</p>;
  }
  return (
    <motion.section 
      className="px-4"
    ...
    >
      <h2 className="text-3xl md:text-4xl font-bold mb-10 md:mb-12 text-center gradient-text-purple-blue">Minhas Redes</h2>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-4xl mx-auto">
        {links.map((link, index) => {
          const IconComponent = iconMap[link.icon?.toLowerCase()] || iconMap.other;
          
          return (
            <motion.div key={index} variants={itemVariants} className="link-card">
              <a 
                href={link.url} 
                target="_blank" 
              ...
              >
                <Button 
                  variant="outline" 
                  className="w-full h-auto py-5 px-6 flex items-center justify-start text-left glass-effect hover:bg-primary/10 group border-border hover:border-primary/40 transition-all duration-300 ease-in-out shadow-lg hover:shadow-primary/20"
                >
                  <div className="mr-4 p-3 rounded-lg bg-primary/10 text-primary group-hover:bg-primary/20 transition-colors border border-primary/20">
                    <IconComponent size={22} />
                  </div>
                  <div>
                    <span className="font-semibold text-lg text-card-foreground group-hover:text-primary transition-colors">{link.title}</span>
                    <p className="text-xs text-muted-foreground line-clamp-1 group-hover:text-primary/80 transition-colors">{link.url?.replace('https://', '')}</p>
                  </div>
                  <Send size={18} className="ml-auto text-muted-foreground opacity-0 group-hover:opacity-100 group-hover:text-primary transition-all duration-300 transform group-hover:translate-x-1" />
                </Button>
              </a>
            </motion.div>
          );
